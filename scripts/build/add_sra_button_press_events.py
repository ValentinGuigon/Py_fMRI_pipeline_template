...
derived from the self_RT and self_keys columns in the BIDS events files.
...
def process_tsv(src_path: Path, dst_path: Path, dry_run: bool) -> int:
    ...
    required = {"trial_type", "onset", "self_RT", "self_keys"}
    ...
    for _, row in choice_rows.iterrows():
        rts = parse_list_column(row["self_RT"])
        keys = parse_list_column(row["self_keys"])
        t0 = float(row["onset"])
...
def main():
    ...
    tsvs = find_events_tsvs(
        args.src_bids_dir, args.subjects or [], args.tasks or [])
    print("Found {} events TSV(s).\n".format(len(tsvs)))

    n_ok = n_skip = n_err = 0
    for src_path in tsvs:
        dst_path = args.dst_events_dir / \
            src_path.relative_to(args.src_bids_dir)
